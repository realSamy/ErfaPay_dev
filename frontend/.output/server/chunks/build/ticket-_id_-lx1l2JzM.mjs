import { b as useI18n, F as useRoute, J as useBreadcrumbStore, k as useAuth, _ as _sfc_main$l, h as _sfc_main$q, V as _sfc_main$p, E as useConfirm } from './server.mjs';
...
import { c as useAdminReplyTicket, d as useAdminUpdateTicket } from './useTickets-CnXAgghv.mjs';
import 'node:http';
import 'node:https';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const ticket = ref();
    const { reply: ticketReply, isLoading: ticketReplyLoading } = useAdminReplyTicket(ticket_id);
    const { update } = useAdminUpdateTicket(ticket_id);
    ...
    const assignTicket = async () => {
      if (!user.value?.id)
        return;
      const { data: response } = await update({ assigned_to: user.value.id });
    ...
    const replyHandler = async () => {
      if (!ticket.value)
        return;
      const response = await ticketReply(payload.value);
      if (response.value?.ok) {
        ticket.value = response.value.data;
        payload.value = {
          message: "",
    ...
    const handleCloseTicket = async () => {
      ...
      const { data: response } = await update({ status: "resolved" });
      if (response.value?.ok)
        ticket.value = response.value.data;
    };
    const handleMarkAsProgress = async () => {
      const confirmed = await useConfirm({
        title: $t("pages.tickets.labels.mark_as_progress"),
        message: $t("pages.tickets.messages.mark_as_progress", [ticket_id]),
        confirmLabel: $t("pages.tickets.labels.change_state"),
      ...
      await assignTicket();
      const { data: response } = await update({ status: "in_progress" });
      if (response.value?.ok)
        ticket.value = response.value.data;
    };
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(ticket)) {
        ...
        _push(ssrRenderComponent(_component_UButton, {
          disabled: ["closed", "resolved", "in_progress"].includes(unref(ticket).status),
          title: unref($t)("pages.tickets.labels.mark_as_progress"),
          color: "warning",
          icon: "material-symbols:pending-actions-rounded",
          size: "xl",
          variant: "soft",
          onClick: handleMarkAsProgress
        }, null, _parent));
        _push(`</div></div></div><div class="space-y-3"><h2 class="font-medium text-center">جزییات تیکت ارسالی</h2><div class="space-y-1"><div class="grid grid-cols-2"><span>زمان ایجاد تیکت:</span><span class="font-bold text-end">${ssrInterpolate(_ctx.$d(new Date(unref(ticket).created_at), fullDate))}</span></div><div class="grid grid-cols-2"><span>زمان آخرین بروزرسانی:</span><span class="font-bold text-end">${ssrInterpolate(_ctx.$d(new Date(unref(ticket).updated_at), fullDate))}</span></div><div class="grid grid-cols-2"><span>آخرین پاسخ توسط:</span><span class="font-bold text-end">${ssrInterpolate(unref(ticket).assigned_to ? unref(ticket).assigned_to.full_name : "-")}</span></div><div class="grid grid-cols-2"><span>${ssrInterpolate(unref($t)("common.tables.category"))}:</span><span class="font-bold text-end">${ssrInterpolate(unref(ticket).category.title_fa)}</span></div><div class="grid grid-cols-2"><span>${ssrInterpolate(unref($t)("pages.tickets.titles.current_state"))}:</span><span class="font-bold text-end">${ssrInterpolate(unref(statusMap)[unref(ticket).status])}</span></div></div></div></div></section>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<section class="h-full flex flex-col grow contain-size">`);
      _push(ssrRenderComponent(_component_UChatPalette, null, {
        prompt: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            } else if (unref(isSendDisabled)) {
              _push2(ssrRenderComponent(_component_UButton, {
                disabled: "",
                label: unref($t)("pages.tickets.messages.is_assigned_to_other"),
                block: "",
                color: "warning"
              }, null, _parent2, _scopeId));
            } else {
            ...
            if (unref(ticket)?.assigned_to) {
              ...
              _push2(ssrRenderComponent(_component_UButton, {
                label: unref($t)("pages.tickets.labels.assign_to_self"),
                loading: unref(ticketReplyLoading),
                block: "",
                class: "text-center",
                color: "success",
                variant: "soft",
            ...
            return [
              ["closed", "resolved"].includes(unref(ticket)?.status || "") ? (openBlock(), createBlock(_component_UButton, {
                ...
                block: "",
                color: "warning"
              }, null, 8, ["label"])) : unref(isSendDisabled) ? (openBlock(), createBlock(_component_UButton, {
                key: 1,
                disabled: "",
                label: unref($t)("pages.tickets.messages.is_assigned_to_other"),
                block: "",
              ...
              unref(ticket)?.assigned_to ? (openBlock(), createBlock(_component_UChatPrompt, {
                key: 2,
                modelValue: unref(payload).message,
                "onUpdate:modelValue": ($event) => unref(payload).message = $event,
                disabled: unref(isSendDisabled),
                ...
                _: 1
              }, 8, ["modelValue", "onUpdate:modelValue", "disabled"])) : (openBlock(), createBlock(_component_UButton, {
                key: 3,
                label: unref($t)("pages.tickets.labels.assign_to_self"),
                loading: unref(ticketReplyLoading),
                block: "",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/support/tickets/ticket-[id].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=ticket-_id_-lx1l2JzM.mjs.map
