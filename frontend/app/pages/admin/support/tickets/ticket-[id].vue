<template>
  <div class="max-h-full h-full flex flex-col lg:flex-row-reverse gap-4 relative">
    <section
        v-if="ticket"
        class="bg-blue-400 text-white dark:bg-blue-900 w-full p-2 h-80 lg:w-[400px] lg:h-full rounded-md shadow-sm">
      ...
      <div class="flex lg:flex-col justify-between gap-6">
        <div class="space-y-4">
          ...
          <div class="flex flex-col lg:flex-row lg:justify-between">
            ...
            <div class="space-x-2">
              <UButton
                  :disabled="['closed', 'resolved'].includes(ticket.status)"
    ...
    <section class="h-full flex flex-col grow contain-size">
      <UChatPalette>
        ...
        <template #prompt>
          ...
          <UButton
              disabled
              v-else-if="isSendDisabled"
              :label="$t('pages.tickets.messages.is_assigned_to_other')"
              block
              color="warning"/>
          <UChatPrompt
          ...
          <UButton v-else :label="$t('pages.tickets.labels.assign_to_self')" :loading="ticketReplyLoading" block
                   class="text-center"
                   color="success" variant="soft" @click="assignTicket"/>
        </template>
      </UChatPalette>
    </section>
  </div>
...
if (!import.meta.server) {
  const {data: response} = await useAdminGetTicket(ticket_id)
  if (response.value?.ok) {
    ticket.value = response.value.data
  }
}
