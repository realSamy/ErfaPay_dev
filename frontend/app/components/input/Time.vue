...
const internalValue = ref<TimeString>('00:00')
const inputRef = ref<HTMLInputElement | null>(null)

// Computed
const formatted = computed(() => {
  const val = internalValue.value || props.modelValue || '00:00'
  return val.replace(/(\d{2}):?(\d{2})/, '$1:$2') // Ensure : format
})
...
const handleBlur = () => {
  ...
  internalValue.value = `${clampedH.toString().padStart(2, '0')}:${clampedM.toString().padStart(2, '0')}` as TimeString;
}
