...
const columns: Ref<TableColumn<Ticket>[]> = ref([
  ...
  // Ticket ID
  {
    ...
    cell: ({row}) => {
      const ticket = row.getValue<Ticket['id']>('id')
      return h(UButton, {
        label: n(ticket, {useGrouping: false}),
        variant: 'link',
        to: localePath({name: 'dashboard-support-ticket-id', params: {id: ticket}})
      })
    },
  },
    {
    ...
    cell: ({row}) => {
      const ticket = row.getValue<Ticket['ticket_id']>('ticket_id')
      return h(UButton, {
        label: useConvertNumericToLocale(ticket, locale.value),
        variant: 'link',
        to: localePath({name: 'dashboard-support-ticket-id', params: {id: ticket}})
      })
    },
  ...
  // Subject
  {
    ...
    cell: ({row}) => {
      ...
      return h(UButton, {
        label,
        variant: 'link',
        to: localePath({name: 'dashboard-support-ticket-id', params: {id: ticket_id}})
      })
    },
  },
  // Category
  ...
  // Assigned To
  {
    ...
    cell: ({row}) => {
      const user = row.getValue<Ticket['assigned_to']>('assigned_to')
